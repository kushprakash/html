                        ...
                        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form class="needs-validation" action="{{ url('sub-sub-category-submit') }}" method="POST" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                             <div class="form">
                                                ...
                                                <div class="form-group mb-0">
                                                    ...
                                                    <select type="text" name="sub_category" value="" id="sub_cat_id" placeholder="SubCategory"   class="form-control" >      
                                                 ...
                                                 <div class="form-group mb-0">
                                                    ...
                                                    <input class="form-control" name="sub_sub_category"type="text">
                                                </div>
                                            </div>
                    ...
                    <td> @php       $menuss=DB::table('header_menu')->where('id',$value->header_name)->first(); @endphp
                     @if($menuss!=null) {{$menuss->menu_name}} @else N.A. @endif</td> 
                      ...
                      <td><?php $category_name=DB::table('category')->where('id',$value->category_id)->first(); ?> @if($category_name!=null){{$category_name->category}} @else N.A. @endif</td> 
                         <td> @php       $menuss=DB::table('sub_category')->where('id',$value->sub_category)->first(); @endphp
                     @if($menuss!=null) {{$menuss->sub_category}} @else N.A. @endif</td>
                    <td>{{$value->sub_sub_category}} </td> 
                    
                          
                            <td>
                                 <a class="btn btn-xs btn-danger" href="{{ url('sub-sub-categorys-delete/'.$value->id) }}">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title f-w-600" id="exampleModalLabel">Edit SubSubCategory</h5>
                                        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="needs-validation" action="{{ url('sub-sub-categoryedit-submit') }}" method="POST" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                             ...
                                             <div class="form">
                                                ...
                                                <div class="form-group">
													...
													<?php $df=DB::table('category')->where('id',$value->category_id)->where('deleted_at',null)->first();?>
                                                      <select  name="category_id"  id="cat_ide" onchange="load_sub_categorye(this.value)"   placehoder="Enter Address" class="form-control w-100 mb-1" >    @if($df != null)  <option value="{{$df->id}}">{{$df->category}}</option> @endif
												 ...
												 <div class="form-group mb-0">
                                                    <label for="validationCustom02" class="mb-1">Sub Category Name:</label>
											<?php 		 $df=DB::table('sub_category')->where('id',$value->sub_category)->where('deleted_at',null)->first();?>
                                                    <select type="text" name="sub_category" value="" id="sub_cat_ide" placeholder="SubCategory"   class="form-control w-100 mb-1" >      
						 @if($df != null)  <option value="{{$df->id}}">{{$df->sub_category}}</option> @endif
					 
						                          </select>
                                                </div>
                                                 <div class="form-group mb-0">
                                                    <label for="validationCustom02" class="mb-1">Sub subCategory Name:</label>
                                                    <input class="form-control w-100 mb-1" name="sub_sub_category" value="{{$value->sub_sub_category}}" type="text">
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                        <button class="btn btn-primary" type="submit" onclick="this.form.submit()">Save</button>
...
xhr.send();
	...
	<script>
		function load_sub_categorye(id) 
    {
      document.getElementById('sub_cat_ide').innerHTML='';
      console.log(id);
      var xhr = new XMLHttpRequest();
      xhr.open('GET','{{ url("admin/json-response1") }}?cat_id='+id,true);
   ...
   console.log(sub_cat_id);
    ...
    op.innerText = 'Select  Sub Category';
    op.setAttribute('selected', 'selected');
    document.getElementById('sub_cat_ide').appendChild(op);
