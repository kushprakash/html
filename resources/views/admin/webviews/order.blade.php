...
<!-- Container-fluid starts-->
<div class="container-fluid">
    	<form action="{{url('report-search-order')}}" method="post">
		@csrf
	<div class="row mb-4">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Order #Games-{{$value->payment_request_id}} 
			...
			<div class="table-responsive " style="border-top:5px solid black;">
			 ...
			 @foreach($orderd as $key=>$item)  
				...
				<div><strong>{{@$productname->name}}</strong>
								<?php $sa=0;
	 $sama=0; $sakdla=[];  $damsa=DB::table('sub_order')->where('order_id',$item->order_id)->where('product_id',$item->product_id)->where('user_id',$item->user_id)->first();  ?> 
				...
				<table class="table table-condensed nomargin" >
                  ...
                  <td>{{$value->wallet}} </td>
                    ...
                    <td>
						...
						<?php $dsa=DB::table('order_history')->where('order_id',$value->order_id)->get();?>
          @if($dsa != "[]")
					@foreach($dsa as $key=>$sr)	 
						
						@if($sr->status==6)
			 Cancelled <strong> ( {{$sr->created_at}} ) </strong> <br>
						@endif
			  
	 					 
		  @if($sr->status!=6)
			        @if($sr->status==1)
						    Food  preparation <strong> ( {{$sr->created_at}} ) </strong> <br>
                            @endif
						
						  @if($sr->status==2)
						  
						   
                           Food is ready & Pick-up <!--Your food is ready. Head on over to the Tuck Shop counter & collect it soon!--><strong> ( {{$sr->created_at}} ) </strong> <br>
                            @endif
						   @if($sr->status==3)
                               Delivered <strong> ( {{$sr->created_at}} ) </strong> <br>
                            @endif 
						   @if($sr->status==4)
						    Order Accept  <strong> ( {{$sr->created_at}} ) </strong> <br>
                            @endif
		 	@endif
						@endforeach
						@endif
					...
					<td>
						...
						<?php $dsa=DB::table('order_history')->where('order_id',$value->order_id)->where('status',6)->first();?>
						...
						<form action="{{url('order-process/'.$value->order_id)}}" id="delivered" method="get">
							  <select name="valu" class="form-control" required>
								  ...
								  <?php $dsaord=DB::table('order_history')->where('order_id',$value->order_id)->where('status',4)->first();?>
						@if($dsaord == null)
								   <option value="4">Order Accept</option>
								  @endif
								    <?php $dsaopri=DB::table('order_history')->where('order_id',$value->order_id)->where('status',1)->first();?>
				@if($dsaopri== null)
								  <option value="1">Food preparation</option>
								  @endif
								<?php  $dsaopick=DB::table('order_history')->where('order_id',$value->order_id)->where('status',2)->first();?>
				@if($dsaopick== null)
								   <option value="2">  Food is ready & Pick-up <!--Your food is ready. Head on over to the Tuck Shop counter & collect it soon!--></option>
							@endif	  
								  <?php $dsaodie=DB::table('order_history')->where('order_id',$value->order_id)->where('status',3)->first();?>
				@if($dsaodie== null)
